...
-- Question 2: View ALL_WORKERS_ELAPSED
...
-- Question 3: View BEST_SUPPLIERS
CREATE OR REPLACE VIEW BEST_SUPPLIERS AS
SELECT supplier_id, supplier_name, SUM(total_pieces_delivered) AS total_pieces_delivered
FROM (
    SELECT s.supplier_id, s.name AS supplier_name, SUM(quantity) AS total_pieces_delivered
    FROM SUPPLIERS_BRING_TO_FACTORY_1 sf1
    JOIN SUPPLIERS s ON sf1.supplier_id = s.supplier_id
    GROUP BY s.supplier_id, s.name
    HAVING SUM(quantity) > 1000
    UNION ALL
    SELECT s.supplier_id, s.name AS supplier_name, SUM(quantity) AS total_pieces_delivered
    FROM SUPPLIERS_BRING_TO_FACTORY_2 sf2
    JOIN SUPPLIERS s ON sf2.supplier_id = s.supplier_id
    GROUP BY s.supplier_id, s.name
    HAVING SUM(quantity) > 1000
) GROUP BY supplier_id, supplier_name
ORDER BY total_pieces_delivered DESC;

-- Question 4: View ROBOTS_FACTORIES
